...
pub type AltStackOption<T, A> = Option<impl AltStackActivator<T, A>>;

pub struct CardButton;

impl CardButton {
  const fn stack_none<T: Data, A: Data>() -> AltStackOption<T, A> {
    Option::<
      fn(
        ScopedStackCardButton<T>,
        Rc<dyn Fn() -> Box<dyn Widget<A>> + 'static>,
  ...
  pub fn stacked_dropdown<T: Data, A: Data, W: Widget<T> + 'static, WO: Widget<A> + 'static>(
  ...
  >(
    ...
    let dropdown: Rc<dyn Fn() -> Box<dyn Widget<A>>> =
  ...
  pub fn trigger_dropdown_manually<T: Data, A: Data>(
    ctx: &mut (impl CommandCtx + LaidOutCtx),
    dropdown: Rc<dyn Fn() -> Box<dyn Widget<A>> + 'static>,
    id: WidgetId,
...
pub type ScopedStackCardButton<T> = druid::widget::LensWrap<
  State<T, bool>,
  T,
  state_derived_lenses::outer<T, bool>,
