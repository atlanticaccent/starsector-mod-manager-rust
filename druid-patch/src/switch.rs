...
use druid::{
  ...
  kurbo::{Circle, Shape},
  piet::{LinearGradient, RenderContext, UnitPoint},
  theme,
  widget::prelude::*,
  ArcStr, Color, KeyOrValue, Point, TextLayout,
};
...
pub struct Switch {
  ...
  height: Option<f64>,
  width: Option<f64>,
  on_color: KeyOrValue<Color>,
  on_color_stop: KeyOrValue<Color>,
  off_color: KeyOrValue<Color>,
  off_color_stop: KeyOrValue<Color>,
}
...
impl Default for Switch {
  fn default() -> Self {
    Switch {
      ...
      height: None,
      width: None,
      on_color: theme::PRIMARY_LIGHT.into(),
      on_color_stop: theme::PRIMARY_DARK.into(),
      off_color: theme::BACKGROUND_LIGHT.into(),
      off_color_stop: theme::BACKGROUND_DARK.into(),
    }
  }
}
...
impl Switch {
  ...
  pub fn on_text(mut self, text: impl Into<ArcStr>) -> Self {
    self.on_text = TextLayout::from_text(text);

    self
  }

  pub fn off_text(mut self, text: impl Into<ArcStr>) -> Self {
    self.off_text = TextLayout::from_text(text);

    self
  }

  pub fn textless(self) -> Self {
    self.on_text("").off_text("")
  }
  ...
  pub fn on_color(mut self, color: impl Into<KeyOrValue<Color>>) -> Self {
    self.on_color = color.into();

    self
  }

  pub fn on_color_stop(mut self, color: impl Into<KeyOrValue<Color>>) -> Self {
    self.on_color_stop = color.into();

    self
  }

  pub fn off_color(mut self, color: impl Into<KeyOrValue<Color>>) -> Self {
    self.off_color = color.into();

    self
  }

  pub fn off_color_stop(mut self, color: impl Into<KeyOrValue<Color>>) -> Self {
    self.off_color_stop = color.into();
  ...
  fn paint_labels(&mut self, ctx: &mut PaintCtx, env: &Env, switch_width: f64) {
    ...
    let switch_height = self
      .height
      .unwrap_or_else(|| env.get(theme::BORDERED_WIDGET_HEIGHT));
    ...
    let on_base_x_pos = -on_label_width - (label_x_space - on_label_width) / 2.0 + SWITCH_PADDING;
...
impl Widget<bool> for Switch {
  fn event(&mut self, ctx: &mut EventCtx, event: &Event, data: &mut bool, env: &Env) {
    let switch_height = self
      .height
      .unwrap_or_else(|| env.get(theme::BORDERED_WIDGET_HEIGHT));
    let switch_width = self
      .width
      .unwrap_or_else(|| switch_height * SWITCH_WIDTH_RATIO);
    let knob_size = switch_height - 2. * SWITCH_PADDING;
  ...
  fn layout(&mut self, ctx: &mut LayoutCtx, bc: &BoxConstraints, _data: &bool, env: &Env) -> Size {
    self.on_text.rebuild_if_needed(ctx.text(), env);
    ...
    let height = self
      .height
      .unwrap_or_else(|| env.get(theme::BORDERED_WIDGET_HEIGHT));
    let width = self.width.unwrap_or_else(|| height * SWITCH_WIDTH_RATIO);
  ...
  fn paint(&mut self, ctx: &mut PaintCtx, data: &bool, env: &Env) {
    ...
    let background_gradient_on_state = LinearGradient::new(
      ...
      UnitPoint::BOTTOM,
      (
        self.on_color.resolve(env).with_alpha(opacity),
        self.on_color_stop.resolve(env).with_alpha(opacity),
      ),
    );
    let background_gradient_off_state = LinearGradient::new(
      ...
      UnitPoint::BOTTOM,
      (
        self.off_color.resolve(env).with_alpha(1. - opacity),
        self.off_color_stop.resolve(env).with_alpha(1. - opacity),
      ),
    );
