...
use druid_patch::table::{
  FlexTable, RowData, TableCellVerticalAlignment, TableColumnWidth, TableData,
};

use crate::{fast_im_map::FastImMap, widgets::ecs::EcsWidget};
...
impl<T: WrapData, W: Widget<T> + 'static> TableData for TableDataImpl<T, W> {
  ...
  fn keys(&self) -> impl Iterator<Item = <Self::Row as druid_patch::table::RowData>::Id> {
    0..self.height()
  }
