...
impl<W: Widget<App>> Controller<App, W> for ModListController {
  fn event(&mut self, child: &mut W, ctx: &mut EventCtx, event: &Event, data: &mut App, env: &Env) {
    if let Event::Command(cmd) = event {
      if let Some(single_use) = cmd.get(ModList::OVERWRITE)
        && let Some((conflict, install_from, entry)) = single_use.take()
      {
          ...
          && conflict.starts_with(install_dir)
        {
          ...
          data
            .runtime
            .spawn(data.installer.install(installer::Request::Resumed(
              Box::new(entry),
              install_from,
              conflict,
            )));
