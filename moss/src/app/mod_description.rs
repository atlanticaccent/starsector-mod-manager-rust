...
use common::{
  ...
  lenses::{ident_rc, Compute, LensExtExt as _},
...
use icons::{CHEVRON_LEFT, DELETE, SYSTEM_UPDATE, TOGGLE_ON};
...
pub struct ModDescription<T = Rc<ModEntry>> {
  entry: T,
...
impl ModDescription {
  ...
  pub fn from_entry_self(entry: Rc<ModEntry>, other: ModDescription<String>) -> Self {
    Self {
      entry,
      crumbs: other.crumbs,
    }
    ...
    Flex::column()
      ...
      .with_flex_child(
        Mask::new(Self::body())
          ...
          .lens(ModDescription::entry.then(ident_rc::<ModEntry>())),
        1.0,
      )
      .with_child(
        Flex::row()
    ...
    Card::builder()
      ...
      .build(
        Flex::column()
          .with_child(id())
          .with_spacer(4.0)
          .with_child(entry_controls())
          .with_child(h2_fixed("Version"))
          .with_child(Label::wrapped_lens(
            ModEntry::version.compute(std::string::ToString::to_string),
          ))
          .with_default_spacer()
          .with_child(
            Maybe::or_empty(|| {
          ...
          .with_child(is_utility())
          .with_child(total_conversion())
          .with_child(dependencies())
...
fn dependencies() -> impl Widget<ModEntry> {
  Flex::column()
    ...
    .with_default_spacer()
    .cross_axis_alignment(druid::widget::CrossAxisAlignment::Start)
    .empty_if(|data: &std::sync::Arc<Vec<super::mod_entry::Dependency>>, _| data.is_empty())
    .lens(ModEntry::dependencies)
}

fn is_utility() -> impl Widget<ModEntry> {
  Flex::column()
    .with_child(
      Flex::row()
...
fn total_conversion() -> impl Widget<ModEntry> {
  Flex::column()
    .with_child(
      Flex::row()
...
fn entry_controls() -> Flex<ModEntry> {
  Flex::row()
    .with_child(
      Card::builder()
        .with_insets((0.0, 8.0))
