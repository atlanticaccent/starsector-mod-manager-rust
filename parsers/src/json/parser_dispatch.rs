...
use winnow::{
  Result,
  ascii::{float, multispace0, till_line_ending},
  combinator::{
...
///
/// most of the times you can ignore the error type and use the default (but
/// this examples shows custom error types later on!)
///
...
fn key<'i, E: ParserError<Stream<'i>> + AddContext<Stream<'i>, StrContext>>(
  ...
  repeat(0.., one_of(AsChar::is_alphanum))
...
fn ws<'i, E: ParserError<Stream<'i>>>(input: &mut Stream<'i>) -> Result<&'i str, E> {
  (
    ...
    .take()
    .parse_next(input) // TODO: allow multiple consecutive comments with or without intervening whitespace
}
...
mod test {
  ...
  fn json_string() {
    ...
    assert_eq!(
      string::<Error>.parse_peek("\"abc\\\"\\\\\\/\\b\\f\\n\\r\\t\\u0001\\u2014\u{2014}def\""),
      Ok(("", "abc\"\\/\x08\x0C\n\r\t\x01——def".to_owned())),
    );
    assert_eq!(
