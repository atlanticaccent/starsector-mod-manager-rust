...
impl<T> Split<T> {
  ...
  pub fn update_split_point(&mut self, size: Size, mouse_pos: Point) {
  ...
  /// Set the split's split point chosen.
  pub fn set_split_point_chosen(&mut self, split_point_chosen: f64) {
    self.split_point_chosen = split_point_chosen;
  }
}
      ...
      match event {
        ...
        Event::MouseUp(mouse) => {
          if mouse.button.is_left() && ctx.is_active() {
            ...
            ctx.submit_notification(DRAGGED.with(self.split_point_chosen));
            ctx.request_paint();
          }
        }
