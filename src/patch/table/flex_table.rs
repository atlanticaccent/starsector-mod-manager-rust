...
use druid::{
    ...
    LifeCycleCtx, PaintCtx, Point, RenderContext, Size, UpdateCtx, Widget, Key,
};
        ...
        for (row_num, row) in self.children.iter_mut().enumerate() {
            if let Some(ref row_starts) = self.row_starts {
                ...
                if let Some(row_painter) = self.row_background.as_mut() {
                    ...
                    row_size.height = row_starts.get(row_num + 1).unwrap_or(&row_size.height) - row_starts[row_num];
