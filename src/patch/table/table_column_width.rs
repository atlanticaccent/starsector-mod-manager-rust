...
impl ComplexTableColumnWidth {
  ...
  pub(crate) fn need_intrinsic_width(&self) -> bool {
    match self {
      ...
      Self::Limited(tcw, min, max) => {
        tcw.need_intrinsic_width() || min.need_intrinsic_width() || max.need_intrinsic_width()
      }
    }
  }
    ...
    'outer: for _ in 0..column_count {
      ...
      let px_per_flex = {
        ...
        for col_num in 0..column_count {
          let (_, _width, fixed_share, flex_share) = column_widths[col_num].resolve_width(
            max_table_width,
      ...
      for col_num in 0..column_count {
        let (limit, width, _fixed_share, _flex_share) = column_widths[col_num].resolve_width(
          max_table_width,
          intrinsic_widths[col_num],
          px_per_flex,
          true,
