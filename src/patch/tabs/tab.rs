    ...
    ensure_for_tabs(&mut self.tabs, &data.policy, &data.inner, |policy, key| {
      ...
      if can_close {
        ...
        let row = Flex::row()
          .with_child(label)
          .with_child(
            close_button.on_click(move |_ctx, data: &mut TabsState<TP>, _env| {
...
impl<TP: TabsPolicy> TabsBody<TP> {
  ...
  fn child(children: &mut [(TP::Key, TabBodyPod<TP>)], idx: usize) -> Option<&mut TabBodyPod<TP>> {
...
impl<TP: TabsPolicy> Tabs<TP> {
  ...
  pub fn tab_index(&self) -> TabIndex {
    let index = match &self.content {
      ...
      TabsContent::Building { index, .. } | TabsContent::Complete { index, .. } => Some(*index),
  ...
  fn make_scope(&self, tabs_from_data: TP, idx: TabIndex) -> WidgetPod<TP::Input, TabsScope<TP>> {
    let tabs_bar = if tabs_from_data.has_tab_bar() {
      Some(TabBar::new(self.axis, self.edge))
    } else {
      None
    };
...
impl<TP: TabsPolicy> Widget<TP::Input> for Tabs<TP> {
  ...
  fn lifecycle(&mut self, ctx: &mut LifeCycleCtx, event: &LifeCycle, data: &TP::Input, env: &Env) {
    if let LifeCycle::WidgetAdded = event {
