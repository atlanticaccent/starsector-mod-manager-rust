...
type IndentFactory<T> = dyn Fn(&T) -> f64;
  ...
  L: Lens<T, bool>,
{
  ...
  expand_lens: L,
  indent: Option<Arc<IndentFactory<T>>>,
  max_label_height: Option<f64>,
  opener_size: Option<Size>,
}
...
impl<T: TreeNode, L: Lens<T, bool> + Clone> Widget<T> for TreeNodeWidget<T, L> {
  fn event(&mut self, ctx: &mut EventCtx, event: &Event, data: &mut T, env: &Env) {
    ...
    let event = match event {
      ...
      Event::Notification(notif) if notif.is(TREE_CHILD_REMOVE_INTERNAL) => {
        ...
        let index = usize::try_from(*notif.get(TREE_CHILD_REMOVE_INTERNAL).unwrap()).unwrap();
  ...
  fn layout(&mut self, ctx: &mut LayoutCtx, bc: &BoxConstraints, data: &T, env: &Env) -> Size {
    ...
    let basic_height = env.get(theme::BASIC_WIDGET_HEIGHT);
    let opener_size = self
      .opener_size
      .unwrap_or((basic_height, basic_height).into());
    let height = self.max_label_height.unwrap_or(basic_height);
    let indent = self
      .indent
      .as_ref()
    ...
    self
      .opener
      .layout(ctx, &BoxConstraints::tight(opener_size), data, env);
    self.opener.set_origin(ctx, Point::ORIGIN);

    // Immediately on the right, the node widget
    let widget_size = self.widget.layout(
      ctx,
      &BoxConstraints::new(Size::new(min_width, 0.), Size::new(max_width, max_height)),
      data,
      env,
    );
    self
      .widget
      .set_origin(ctx, Point::new(opener_size.width + indent, 0.0));

    // This is the computed size of this node. We start with the size of the widget,
    // and will increase for each child node.
    let mut size = Size::new(
      indent + widget_size.width,
      opener_size.height.max(widget_size.height),
    );
    ...
    if self.expand_lens.get(data) && max_width > indent {
      ...
      for (index, child_widget_node) in self.children.iter_mut().enumerate() {
        ...
        if index != next_index {
          size.height += (index - next_index) as f64 * height;
        }
        next_index = index + 1;
...
impl<T: TreeNode, L: Lens<T, bool> + Clone + 'static> Tree<T, L> {
  ...
  pub fn with_opener<W: Widget<T> + 'static>(mut self, closure: impl Fn() -> W + 'static) -> Self {
  ...
  fn get_chroot_from<'a>(
    ...
    match data.get_chroot() {
      Some(idx) => {
        Tree::<T, L>::get_chroot_from(&mut widget.widget_mut().children[idx], data.get_child(idx))
      }
      None => (widget, data),
    }
  }

  pub fn with_max_label_height(mut self, height: f64) -> Self {
    self.root_node.widget_mut().max_label_height = Some(height);

    self
  }

  pub fn with_indent(mut self, indent: impl Fn(&T) -> f64 + 'static) -> Self {
    self.root_node.widget_mut().indent = Some(Arc::new(indent));

    self
  }

  pub fn with_opener_dimensions(mut self, size: impl Into<Size>) -> Self {
    self.root_node.widget_mut().opener_size = Some(size.into());
